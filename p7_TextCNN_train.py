...
tf.app.flags.DEFINE_string("traning_data_path","./data/data_train_accusation.txt","path of traning data.") #sample_multiple_label.txt-->train_label_single100_merge
...
def main(_):
    ...
    train, test= load_data_multilabel(FLAGS.traning_data_path,vocabulary_word2index, vocabulary_label2index,FLAGS.sentence_len,name_scope=FLAGS.name_scope)
    trainX, trainY = train
    testX, testY = test
    ...
    with tf.Session(config=config) as sess:
        ...
        for epoch in range(curr_epoch,FLAGS.num_epochs):
            ...
            for start, end in zip(range(0, number_of_training_data, batch_size),range(batch_size, number_of_training_data, batch_size)):
                ...
                ########################################################################################################
                if start%(400*FLAGS.batch_size)==0: # eval every 3000 steps.
                    eval_loss, macro_f1, micro_f1 = do_eval(sess, textCNN, testX, testY,iteration)
                    print("Epoch %d Validation Loss:%.3f\tmacro_f1:%.3f\tmicro_f1:%.3f" % (epoch, eval_loss, macro_f1, micro_f1))
...
def do_eval(sess,textCNN,evalX,evalY,iteration):
    ...
    for start,end in zip(range(0,number_examples,batch_size),range(batch_size,number_examples,batch_size)): #TODO TODO TODO
        ...
        curr_eval_loss, logits= sess.run([textCNN.loss_val,textCNN.logits],feed_dict)#logits：[batch_size,label_size]
        label_predict=[np.argmax(logits[0])] #only select top1
        label_target=transfomr_to_dense_label(evalY[start:end][0]) #TODO TODO TODD0
        print("label_predict:",label_predict,";label_target:",label_target)
        macro_f1=f1_score_fn(label_target, label_predict, average='macro') #y_true should like this:[0, 1, 2, 0, 1, 2]
        micro_f1=f1_score_fn(label_target, label_predict, average='micro')
        eval_macro_f1,eval_micro_f1,eval_counter=eval_macro_f1+macro_f1,eval_micro_f1+micro_f1,eval_counter+1

    return eval_loss/float(eval_counter),float(eval_macro_f1)/float(eval_counter),float(eval_micro_f1)/float(eval_counter)

def transfomr_to_dense_label(label_list):
    result_label=[]
